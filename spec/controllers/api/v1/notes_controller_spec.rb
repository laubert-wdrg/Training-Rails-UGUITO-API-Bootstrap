  ...
  describe 'GET #index' do
    ...
    context 'when there is a user logged in' do
      ...
      context 'when fetching notes using filters' do
        ...
        let(:notes_expected) { notes_custom.sort_by(&:created_at).reverse }
      ...
      context 'when fettching notes with asc order' do
        let(:order) { :asc }
        let(:notes_expected) { user_notes.sort_by(&:created_at) }

        before { get :index, params: { order: order } }
      ...
      context 'when fettching notes with desc order' do
        let(:order) { :desc }
        let(:notes_expected) { user_notes.sort_by(&:created_at).reverse }
